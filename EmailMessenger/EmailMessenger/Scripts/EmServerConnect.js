...
//var Url_GetConnectionList = Url_Base + '/Mail/GetConnectionList';
...
function GetConnectionList() {
    ...
    //$('#ServerConnectForm').addClass('hide');
    $.ajax({
        url: Url_GetConnectionList, //'/Mail/GetDatabaseList',
        type: 'GET',        
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        timeout: 120000, //2 min 3000,= 3 sec
        success: function (result) {
            ...
            if (result.message === "ok") {
                ...
                generateDynamicTable(result.data);
                alert(result.message);
            } else {
                $('#ServerConnectForm').removeClass('hide');
                alert(result.message);
...
function generateDynamicTable(myContacts) {
    ...
    if (noOfContacts > 0) {
        ...
        var divContainer = document.getElementById("connectionList");
